...
const accommodationCsvPath = path.join(__dirname, 'online_tour_guide', 'accommodation.csv');
...
app.post('/api/accommodation', (req, res) => {
    ...
    readFile(accommodationFilePath, (err, accommodations) => {
        ...
        writeFile(accommodationFilePath, accommodations, (err) => {
            ...
            // Convert the updated accommodations array to CSV format
            try {
                const csv = parse(accommodations);
                fs.writeFileSync(accommodationCsvPath, csv); // Save CSV file
                console.log('Accommodation data saved to CSV file.');
            } catch (err) {
                console.error('Error writing to accommodation.csv:', err);
                return res.status(500).json({ message: 'Error saving accommodation data to CSV' });
            }
